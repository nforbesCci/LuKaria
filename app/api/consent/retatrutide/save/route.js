...
export async function POST(request) {
  try {
    ...
    console.log('✅ Retatrutide consent saved successfully. Complete:', document.complete, 'Result:', result);

    return NextResponse.json({
      success: true,
      message: 'Retatrutide consent saved successfully',
      data: document,
    });

  } catch (error) {
    console.error('❌ Error saving retatrutide consent:', error);
    return NextResponse.json(
      { 
        error: 'Failed to save retatrutide consent',
