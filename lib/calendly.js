...
export async function listEventTypes(token, { userUri, organizationUri } = {}) {
  ...
  return (data.collection || []).map((et) => ({
    ...
    locations: Array.isArray(et.locations) ? et.locations : [],
  }));
}

/** Fetch a single event type (includes locations for Scheduling API booking). */
export async function getEventType(token, eventTypeUri) {
  const id = String(eventTypeUri || '').trim().split('/').pop();
  if (!id) throw new Error('Invalid Calendly event type URI');
  const data = await calendlyFetch(`/event_types/${id}`, { token });
  return data.resource;
}

/**
 * Build POST /invitees `location` from the event type's configured locations.
 * Calendly requires `location` (not location_configuration) when the type has one.
 * @see https://developer.calendly.com/schedule-events-with-ai-agents
 */
export function buildInviteeLocation(eventType, { locationDetail } = {}) {
  const locations = eventType?.locations;
  if (!Array.isArray(locations) || locations.length === 0) return null;

  const configured = locations[0];
  const kind = configured?.kind;
  if (!kind) return null;

  const payload = { kind };
  const detail = String(
    locationDetail || configured.location || configured.additional_info || '',
  ).trim();

  if (kind === 'ask_invitee' || kind === 'outbound_call') {
    if (!detail) {
      throw new Error(
        kind === 'outbound_call'
          ? 'A phone number is required to book this appointment. Add your phone in Profile, then try again.'
          : 'Please add a phone number or location in Profile before booking this appointment type.',
      );
    }
    payload.location = detail;
  } else if ((kind === 'custom' || kind === 'physical') && detail) {
    payload.location = detail;
  }
...
export async function createInvitee(
  ...
  const eventType = await getEventType(token, eventTypeUri);
  const location = buildInviteeLocation(eventType, { locationDetail });
