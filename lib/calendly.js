 ...
 * (e.g. https://calendly.com/you/30min ΓåÆ https://api.calendly.com/event_types/ΓÇª).
 */
...
export async function listAvailableTimes(token, { eventTypeUri, startTime, endTime }) {
  ...
  if (!String(eventTypeUri).includes('api.calendly.com/event_types/')) {
    throw new Error(
      'Invalid Calendly event type URI. Import types from Calendly in System Settings ΓåÆ Calendar so each type has an API URI.',
    );
  }
  ...
  // Past start_time ΓåÆ Calendly 400 "The supplied parameters are invalid"
  if (startMs < now) {
    startMs = now + 60 * 1000;
  }
...
export async function createInvitee(
  ...
  if (!String(eventTypeUri).includes('api.calendly.com/event_types/')) {
    throw new Error(
      'Invalid Calendly event type URI. Import types from Calendly in System Settings ΓåÆ Calendar.',
    );
  }
