...
/** Strip accidental wrapping quotes / newlines from env values (common Vercel paste issue). */
...
function isEnergyNutrientId(id) {
  const s = String(id ?? '').trim();
  // FDC nutrientNumber 208 / nutrientId 1008 = Energy (kcal)
  return s === '208' || s === '1008';
}

/** Prefer Energy in kcal (nutrient 208 / id 1008); ignore kJ when kcal is present. */
export function extractEnergyKcalFromNutrients(nutrients) {
  ...
  const normalized = nutrients.map((n) => ({
    number: n.nutrientNumber ?? n.nutrient?.number,
  ...
  const kcal =
    normalized.find(
      (n) =>
        (isEnergyNutrientId(n.number) || isEnergyNutrientId(n.id) || n.name === 'energy') &&
        n.unit === 'KCAL' &&
