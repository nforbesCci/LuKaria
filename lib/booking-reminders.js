...
export async function insertBookingReminderNotification(userId, { dayIso, setBy } = {}) {
  ...
  if (existing) {
    // Doctor sent a fresh reminder window — reopen today's notice if it was completed.
    if (existing.completed || existing.read) {
      const now = new Date();
      await db.collection('NotificationCollection').updateOne(
        { _id: existing._id },
        {
          $set: {
            read: false,
            ...
            setBy: setBy || existing.setBy || null,
      ...
      return {
        inserted: true,
        reopened: true,
        notification: { ...existing, read: false, completed: false },
      };
    }
...
/**
 * Turn off booking reminder alerts and mark related notifications complete
 * after the patient schedules an appointment. Doctor must set a new reminder
 * window to resume alerts.
 */
export async function completeBookingRemindersForUser(
  userId,
  ...
  const profileResult = await db.collection('profiles').updateOne(
    { userId, 'nextBookingReminder.active': true },
    {
      $set: {
        'nextBookingReminder.active': false,
        'nextBookingReminder.completedAt': now.toISOString(),
        'nextBookingReminder.completedReason': reason,
        updatedAt: now,
      },
    },
  );
  const notificationResult = await db.collection('NotificationCollection').updateMany(
    {
      userId,
      type: BOOKING_REMINDER_TYPE,
      $or: [{ completed: { $ne: true } }, { read: { $ne: true } }],
    },
    {
      $set: {
        read: true,
  ...
  return {
    cleared: true,
    reminderDeactivated: profileResult.modifiedCount > 0,
    notificationsCompleted: notificationResult.modifiedCount,
  };
}
