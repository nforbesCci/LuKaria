  ...
  try {
    canvas = await html2canvas(element, {
      ...
      onclone: (_doc, cloned) => {
        ...
        // Lab Requisition 2 already mirrors values in .lrf-line-text; drop those
        // inputs so capture does not paint the same text twice.
        cloned.querySelectorAll('input').forEach((input) => {
          ...
          if (input.closest('.lrf-line-field')) {
            input.remove();
            return;
          }
